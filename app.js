...
function obtenerTipoEquipo(denominacion,tipoManual=''){const frase=normalizarFrase(denominacion);if(!frase)return tipoManual||'Sin clasificar';if(contieneFraseCompleta(frase,'ESTANQUE'))return 'ESTANQUE';if(contieneFraseCompleta(frase,'BOMBA'))return 'BOMBA';if(tipoManual)return tipoManual;for(const tipo of [...TIPOS_BASE,...tiposUsuario])if(contieneFraseCompleta(frase,tipo))return tipo;for(const r of reglasTipoUsuario)if(normalizarFrase(r.buscar)===frase)return r.tipo;return 'Sin clasificar';}
