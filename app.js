...
function guardarWizard(){const p=pendientes[pendienteIndex];if(!p)return alert('No hay un equipo pendiente para guardar.');let unidad=$('wizardUnidad').value;if(unidad==='__NUEVA__'){unidad=$('wizardNuevaUnidad').value.trim().toUpperCase();if(!unidad)return alert('Escribe el nombre de la nueva unidad.');if(!unidadesUsuario.includes(unidad)){unidadesUsuario.push(unidad);nombresUnidades[unidad]=unidad;localStorage.setItem(KEY_UNIDADES,JSON.stringify(unidadesUsuario));localStorage.setItem(KEY_NOMBRES,JSON.stringify(nombresUnidades));}}if(!unidad)return alert('Selecciona una unidad.');const clave=normalizarFrase(p.equipo);unidadesDenominacion[clave]=unidad;localStorage.setItem(KEY_DEN_UNIDADES,JSON.stringify(unidadesDenominacion));cargarFiltroUnidades();aplicarFiltros();pendientes=getPendientes();if(pendienteIndex>=pendientes.length)pendienteIndex=pendientes.length-1;renderWizard();}
