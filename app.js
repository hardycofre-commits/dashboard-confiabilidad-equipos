...
function setupEventos(){
  ...
  if($('planEstado'))$('planEstado').onchange=renderPlanAnual;
  if($('btnLimpiarPlan'))$('btnLimpiarPlan').onclick=()=>{$('planBuscar').value='';$('planMes').value='';$('planEstado').value='';renderPlanAnual();};
}
...
async function cargarPlanAnual(a){archivoPlanAnual=a.name;const rows=await leerExcel(a.download_url,'json');planAnual=rows.map(normalizarFilaPlan).filter(x=>x.fecha||x.equipo||x.plan);cargarMesesPlan();renderPlanAnual();}
...
function renderPlanAnual(){
  ...
  const texto=normalizar($('planBuscar').value),mes=$('planMes').value,estado=$('planEstado').value;
...
function normalizarNombreTipo(tipo){const nombre=String(tipo||'').trim(),clave=normalizarFrase(nombre);if(clave.startsWith('estanque'))return 'ESTANQUE';if(clave.startsWith('bomba'))return 'BOMBA';return nombre;}
...
function llenarTipos(){const select=$('wizardTipoSelect'),tipos=[...new Set(['No aplica',...obtenerListaTipos()])];select.innerHTML='<option value="">Seleccionar tipo</option>'+tipos.map(tipo=>`<option value="${escapeHtml(tipo)}">${tipo==='No aplica'?'NO APLICA':escapeHtml(tipo)}</option>`).join('')+'<option value="__NUEVO__">➕ Nuevo tipo...</option>';}
